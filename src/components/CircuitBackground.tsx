...
// 1. مكون الخلفية المتحركة (محسّن للأداء ومحصور في قسم الـ Hero)
export const CircuitBackground: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  ...
  useEffect(() => {
    ...
    let width = (canvas.width = parent
      ? parent.offsetWidth
      : window.innerWidth);
    let height = (canvas.height = parent
      ? parent.offsetHeight
      : window.innerHeight);
    ...
    // عدم تفعيل الاستماع للمتاوس في الشاشات اللمسية لتوفير المعالج
    if (!isMobile) {
      window.addEventListener("mousemove", handleMouseMove);
    }
    window.addEventListener("resize", handleResize);

    // تقليل عدد الجسيمات في الموبايل
    const particlesCount = isMobile ? 18 : 40;
    const particles: Particle[] = [];
    ...
    class Particle {
      ...
      draw() {
        ...
        // إلغاء الظلال الثقيلة على الموبايل
        if (!isMobile) {
          ctx.shadowBlur = 8;
          ctx.shadowColor = this.color;
        }
    ...
    // إيقاف الـ Canvas فور الخروج من الشاشة لتسريع باقي الموقع
